...
`<div>` 和 `<span>` 是没有语义的，它们就是两种盒子，用来**对网页进行布局和装其他内容**。
...
- 属性之间**不分先后顺序**，标签名与属性、属性与属性之间均以**==空格==**分开（其它标签也是同理）
- **属性均采取键值对**的格式，即：`key="value"` 的格式，`属性="属性值"`
- 设置图像的宽度与高度时：一般设置其中之一便可，另外一个会自动按比例适配
- 设置宽高时，可以使用**百分数**作为值，此时图片大小会以当前父元素的大小为基础进行**比例缩放**，这样做的好处在于当父元素改变大小时，图片也会随比例同等缩放
...
| 相对路径分类 | 符号  | 说明                                                       |
| ------------ | ----- | ---------------------------------------------------------- |
| 同一级路径   | `.`   | 如：`<img src="baidu.png" />`或者<`img src="./baidu.png"`> |
...
绝对路径：指目录下的==绝对位置==，直接到达目的位置，通常是==从盘符开始==的路径。
...
<audio src="D:\背景音乐 (V0).mp3" controls autoplay loop></audio>
```
...
| src      | 音频的路径                                           |
| controls | 显示播放的控件                                       |
| autoplay | 自动播放（部分浏览器不支持）谷歌不支持，功能有局限性 |
| loop     | 循环播放                                             |

播放控件：

![](http://m.qpic.cn/psc?/V51yY6Ml4BjqhQ1s5LEr3CMCXc33WKgl/ruAMsa53pVQWN7FLK88i5ljqoEUaHTnnxmQjAmOQqWQmEe8ucJhB12Ki7ziH.1ahJ7HLoDnkRHzWe5h5EIG46q0U0NICceD2wbqmgEBgw*I!/b&bo=eAE4AHgBOAADCSw!&rf=viewer_4)
...
```html
<vidio src="视频的路径" controls></vidio>

<!-- 谷歌浏览器可以自动静音播放-->
<video src="F:\WORK\B站下载视频\“我爱你，如鲸向海，不可避免，退无可退” - 【鲸目混剪】我爱你，如鲸向海，不可避免，退无可退.mp4" controls autoplay muted></video>
```
...
| src      | 视频的路径                                                   |
| controls | 显示播放的控件                                               |
| autoplay | 自动播放（部分浏览器不支持）谷谷歌浏览器中需配合 muted 实现静音播放 |
| loop     | 循环播放                                                     |

![](http://m.qpic.cn/psc?/V51yY6Ml4BjqhQ1s5LEr3CMCXc33WKgl/ruAMsa53pVQWN7FLK88i5pRENatC798M.teG7gF3APOHKGovUniq4ul7x2JfXdwr.QiD4qyOp8qoEsX5xLb*y*tW7WfQbTw*KDEZ88pFDSY!/b&bo=9wPJAfcDyQEDCSw!&rf=viewer_4)
...
<a href="https://www.bilibili.com/video/BV1vB4y1t7uQ?spm_id_from=333.851.b_7265636f6d6d656e64.3">超链接</a>
```
...
当开发网页初期的时候，我们还不知道跳转地址的时候，href 的值书写为 # （空链接）
...
在表格标签中，分别用：`<thead>` 标签表示表格的头部区域，`<tbody>` 标签表示表格的主体区域，这样可以更好的分清表格结构。    这样有助于浏览器渲染网页。
...
<u>在实际开发中凡是遇到排列整齐的简洁内容，都可以使用列表来进行布局。</u>
...
```html
    <form action="">
        <!-- ”text” 书写什么就显示什么-->
        文本框：<input type="text" placeholder="输入文本">
...
| 属性名      | 属性值       | 描述                                                         |
| ----------- | ------------ | ------------------------------------------------------------ |
| `name`      | 由用户自定义 | 定义 input 元素的名称。分组。有相同name属性值的单选框为一组，一组中同时只能有一个被选中 |
| `value`     | 由用户自定义 | 规定 input 元素的值，也就是提交到服务器的值                  |
| `checked`   | checked      | 规定此 input 元素首次加载时应当被选中（默认选中）            |
...
方法一：
        ...
        <br>
方法二：
        <label><input type="radio" name="se">男</label>
        <label><input type="radio" name="se">女</label>
```
