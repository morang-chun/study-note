...
如果想要**差异化选择不同的标签**，单独选一个或者某几个标签，可以使用 `类选择器` 。
...
- 不能使用已有的关键字作为类名
- 不要使用纯数字、中文等命名，尽量使用**英文字母**来表示
- **命名要有意义**，尽量使别人一眼就知道这个类名的目的（**可读性第一，长度第二，推荐使用英语，如果是使用拼音请使用全拼**）ss
...
1. 并集选择器的每组选择器之间通过 `,` 分隔
2. 并集选择器的每组选择器可以是基础选择器或者符合选择器
...
- （既又原则） 找到页面中 **既** 能够被选择器1选中，**又** 能被选择器2选中的标签，设置样式

注意点：

1. 交集选择器中的选择器之间是紧挨着的，没有东西分隔
2. 交集选择器中如果有标签选择器，**标签选择器**必须写在最前面
...
![](https://wx1.sinaimg.cn/large/008uF2zMly8h58h0lrjddj31z4140hdt.jpg)
...
| rgba           | rgba(255,255,255,0.5)  红绿蓝三原色 + a 表示透明度，取值范围是 0~1 |
...
![](https://wx3.sinaimg.cn/large/008uF2zMly8h58h0euqq4j308305iwel.jpg)
